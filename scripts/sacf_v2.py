...
class Janela(QMainWindow):
    ...
    def analisar(self,empresa_name):
        ...
        if empresa_name == "ARTE BRILHO":
            ...
            df_fgts[1] = df_fgts[1].str.replace('[.-]', '', regex=True)
            """
        ...
        if empresa_name == "CRESCER":
                ...
                for i, line in enumerate(lines):
                    if (salarios_enviados % n_profs != 0 or salarios_enviados == 0) or (cargos_enviados % n_profs != 0 or cargos_enviados == 0):
                        if line == "05118764000108 CNPJ/CEI: " or "Base IRRF Folha" in line:
                            ...
                            matricula = lines[i+1][0:6]
                            matriculas.append(matricula)
                            matriculas_enviadas += 1
            ...
            df_fol_an = pd.DataFrame({'Nome': nomes, 'Salário Liquido': salarios,'Cargo': cargos, 'Data_Admissão': data_adm, 'Matrícula': matriculas})
            """ 
        ...
        for i, line in enumerate(aba_mdo['D']):
            if i >= 13:
                ...
                matricula = aba_mdo[f'A{i+1}']
                matriculas.append(matricula.value)
        ...
        lista_comparacao = [
            ...
            {'df':df_fgts,'titulo':"FGTS",'coluna_nome':0,'coluna_codigo':1},
            {'df':df_fol_an,'titulo':"Folha analítica",'coluna_nome':"Nome",'coluna_valor':'Salário Liquido','coluna_codigo':'Matrícula'}
        ]

        df_relatorio_erros = pd.DataFrame(columns=['conferencia','nome','erro'])

        # verificação --> Planilha -> Folha analítica > Profissional | Cargo | Data de admissão
        doc = lista_comparacao[2] #folha analítica
        ...
        for linha in df_conf_sal.values:
            ...
            conf_sal_matricula = linha[6]

            conf_sal_matricula =  str(conf_sal_matricula).zfill(6)

            resultado = doc['df'].loc[doc['df'][doc['coluna_codigo']].str.contains(conf_sal_matricula),doc['coluna_codigo']]
            ...
            if resultado.empty:
                ...
                df_erro = pd.DataFrame([['Planilha -> Folha analítica',conf_sal_nome,'Profissional não encontrado pela matrícula']],columns=['conferencia','nome','erro'])
                df_relatorio_erros = pd.concat([df_relatorio_erros,df_erro],ignore_index=True)
            else:
                if len(resultado) == 1:
                    cargo = doc['df'].loc[doc['df'][doc['coluna_codigo']].str.contains(conf_sal_matricula),'Cargo'].values[0]
                    data_adm = doc['df'].loc[doc['df'][doc['coluna_codigo']].str.contains(conf_sal_matricula),'Data_Admissão'].values[0]
        ...
        doc = lista_comparacao[0] #comprovante de pagamento

        for i, matricula in enumerate(df_fol_an['Matrícula']):
            ...
            for j, matricula_conf_sal in enumerate(df_conf_sal['Matricula']):
                matricula_conf_sal = str(matricula_conf_sal).zfill(6)
                if matricula == matricula_conf_sal:
                    cpf = df_conf_sal['CPF'][j]
                    nome = df_conf_sal['Nome'][j]

                    if cpf is None or len(cpf) < len('000.000.000-00'):
                        cpf = '000.000.000-00'
            ...
            resultado = doc['df'].loc[doc['df'][doc['coluna_nome']].str.contains(cpf),doc['coluna_nome']]

            if resultado.empty: 
                df_erro = pd.DataFrame([['Folha analítica -> Comprovante de pagamento',df_fol_an['Nome'][i],'Erro no CPF ou matrícula']],columns=['conferencia','nome','erro'])
                df_relatorio_erros = pd.concat([df_relatorio_erros,df_erro],ignore_index=True)
            else:
                if len(resultado) == 1:
                    valor = doc['df'].loc[doc['df'][doc['coluna_nome']].str.contains(cpf),doc['coluna_valor']].values[0]
        ...
        for linha in df_conf_sal.values:
            ...
            conf_sal_pis = str(conf_sal_pis).zfill(len('12703171112'))

            resultado = doc['df'].loc[doc['df'][doc['coluna_codigo']].str.contains(conf_sal_pis),doc['coluna_codigo']]

            if resultado.empty: 
                df_erro = pd.DataFrame([['Planilha -> GFIP - SEFIP',conf_sal_nome,'Profissional não encontrado pelo PIS/PASEP']],columns=['conferencia','nome','erro'])
                df_relatorio_erros = pd.concat([df_relatorio_erros,df_erro],ignore_index=True)
            else:
                if len(resultado) == 1:
        ...
        if not df_relatorio_erros.empty:
            ...
            self.colocarTexto("Faturamento sem erros!")
            return('norel')
